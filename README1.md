...
Two endpoints turn the narrative half of the workspace into the production half:

- **Scenes** — `break story into scenes` on the Scenes page reads the story, the director's
  vision, the Visual DNA, the world bible and any script, and proposes scenes.
- **Shots** — `break into shots` on a scene reads that scene in full, on top of the whole-film
  context, and proposes coverage with camera, light, motion and emotion filled in.

Both are two-phase. The first call **proposes and writes nothing**; you review the proposals in
a modal, untick what you don't want, edit any field in place, and only then are rows created.
Proposals that name a character, location or prop by its exact name in the world bible are
linked to it on apply, so continuity is inherited from the first frame. Anything **locked** is
declared as fixed in the context and is never proposed against.

Without an API key both fall back to the structural provider: scenes come from the script's
scene headings, or from your story's own paragraph breaks; shots come from the scene
description split at its sentences, with camera and light left **empty** rather than invented.
